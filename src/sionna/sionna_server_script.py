...
import numpy as np
import socket
from sionna.rt import load_scene, PlanarArray, Transmitter, Receiver, PathSolver, Camera, RadioMapSolver
...
def manage_location_message(message, sionna_structure):
    ...
    try:
        ...
        sionna_structure["SUMO_live_location_db"][dev] = {"x": new_x, "y": new_y, "z": new_z, "angle": new_angle, "v_x": new_v_x, "v_y": new_v_y, "v_z": new_v_z}

        # 0 - Is devect in sionna_location_db?
        if dev in sionna_structure["sionna_location_db"] and dev[:3] == "car":
            # Fetch the old values
            old_x = sionna_structure["sionna_location_db"][dev]["x"]
            old_y = sionna_structure["sionna_location_db"][dev]["y"]
            old_z = sionna_structure["sionna_location_db"][dev]["z"]
            old_angle = sionna_structure["sionna_location_db"][dev]["angle"]
            ...
            if sionna_structure["verbose"] and (position_changed or angle_changed):
                print(f"Update needed for {dev} - Position changed: {position_changed} - Angle changed: {angle_changed}")
        
        else:
            # First update ever
            if sionna_structure["verbose"]: 
                print(f"First update ever for {dev} - No previous values found. Forcing update...")
        ...
        if  position_changed or angle_changed:
            sionna_structure["sionna_location_db"][dev] = sionna_structure["SUMO_live_location_db"][dev]
            ...
            if  sionna_structure["scene"].get(dev[:3]+"_"+dev[3:]):
                from_sionna = sionna_structure["scene"].get(f"car_{dev[3:]}")
                ...
                if sionna_structure["verbose"]: 
                    print(f"Updated {dev} position in the scene.")

            elif dev[:3] != "enb":
                print(f"ERROR: no {dev} in the scene, use Blender to check")

            sionna_structure["scene"].remove(f"{dev}_tx_antenna")
            sionna_structure["scene"].remove(f"{dev}_rx_antenna")
            if sionna_structure["verbose"]:
                print(f"Removed antennas for {dev} from the scene.")
...
def match_rays_to_devs(paths, sionna_structure):
    t = time.time()
    matched_paths = {}
    ...
    # Adjust dev positions for antenna displacement
    adjusted_dev_locs = {
        dev_id: {
            "x": dev_loc["x"] + sionna_structure["antenna_displacement"][0],
            "y": dev_loc["y"] + sionna_structure["antenna_displacement"][1],
            "z": dev_loc["z"] + sionna_structure["antenna_displacement"][2], 
        }
        for dev_id, dev_loc in sionna_structure["sionna_location_db"].items()
    }

    dev_ids = list(adjusted_dev_locs.keys())
    dev_positions = np.array([[v["x"], v["y"], v["z"]] for v in adjusted_dev_locs.values()])
    dev_tree = cKDTree(dev_positions)

    # Match sources and targets
    source_dists, source_indices = dev_tree.query(sources, distance_upper_bound=sionna_structure["position_threshold"])
    target_dists, target_indices = dev_tree.query(targets, distance_upper_bound=sionna_structure["position_threshold"])


    for tx_idx, src_idx in enumerate(source_indices):
        if src_idx == len(dev_ids):
            if sionna_structure["verbose"]:
                print(f"Warning - No dev within tolerance for source {tx_idx}")
            continue

        matched_source_dev_name = dev_ids[src_idx]
        if matched_source_dev_name not in matched_paths:
            matched_paths[matched_source_dev_name] = {}

        for rx_idx, tgt_idx in enumerate(target_indices):
            if tgt_idx == len(dev_ids):
                if sionna_structure["verbose"]:
                    print(f"Warning - No dev within tolerance for target {rx_idx} (for source {tx_idx})")
                continue

            matched_target_dev_name = dev_ids[tgt_idx]
            if matched_target_dev_name not in matched_paths[matched_source_dev_name]:
                matched_paths[matched_source_dev_name][matched_target_dev_name] = {
                    'path_coefficients': [],
                    'delays': [],
                    'is_los': []
                }
            ...
            try:
                ...
                matched_paths[matched_source_dev_name][matched_target_dev_name]['path_coefficients'].append(coeff)
                matched_paths[matched_source_dev_name][matched_target_dev_name]['delays'].append(delay)
                matched_paths[matched_source_dev_name][matched_target_dev_name]['is_los'].append(bool(is_los))
...
def compute_rays(sionna_structure):
    ...
    # Ensure every car in the simulation has RX and every enb has TX
    for enb_id in sionna_structure["sionna_location_db"]:
        if enb_id[:3] == "car":
            rx_antenna_name = enb_id+"_rx_antenna"
            car_position = np.array(
                [sionna_structure["sionna_location_db"][enb_id]['x'], 
                 sionna_structure["sionna_location_db"][enb_id]['y'],
                 sionna_structure["sionna_location_db"][enb_id]['z']])
            ...
            if sionna_structure["scene"].get(rx_antenna_name) is None:
                sionna_structure["scene"].add(Receiver(rx_antenna_name, position=rx_position, orientation=[0, 0, 0], display_radius=1, color=[0, 0, 1]))
                sionna_structure["scene"].rx_array = sionna_structure["scene"].rx_array
                if sionna_structure["verbose"]:
                    print(f"Added RX antenna for car: {rx_antenna_name}")

        elif enb_id[:3] == "enb":
            tx_antenna_name = enb_id+"_tx_antenna"
            ...
            if sionna_structure["scene"].get(tx_antenna_name) is None:
                sionna_structure["scene"].add(Transmitter(tx_antenna_name, position=tx_position, orientation=[0, 0, 0], display_radius=2, color=[1, 0, 0]))
                sionna_structure["scene"].tx_array = sionna_structure["scene"].tx_array
                if sionna_structure["verbose"]:
                    print(f"Added TX antenna for enb: {tx_antenna_name}")
    ...
    t = time.time()
    matched_paths = match_rays_to_devs(paths, sionna_structure)
    if sionna_structure["time_checker"]:
        print(f"Matching rays to devs took: {(time.time() - t) * 1000} ms")

    # Iterate over sources in matched_paths
    for src_dev_id in sionna_structure["sionna_location_db"]:
        current_source_dev_name = src_dev_id
        if current_source_dev_name in matched_paths:
            matched_paths_for_source = matched_paths[current_source_dev_name]

            # Iterate over targets for the current source
            for trg_dev_id in sionna_structure["sionna_location_db"]:
                current_target_dev_name = trg_dev_id
                if current_target_dev_name != current_source_dev_name:  # Skip case where source == target
                    if current_target_dev_name in matched_paths_for_source:
                        if current_source_dev_name not in sionna_structure["rays_cache"]:
                            sionna_structure["rays_cache"][current_source_dev_name] = {}
                        # Cache the matched paths for this source-target pair
                        sionna_structure["rays_cache"][current_source_dev_name][current_target_dev_name] = \
                            matched_paths_for_source[current_target_dev_name]
                        if sionna_structure["verbose"]:
                            print(
                                f"Cached paths for source {current_source_dev_name} to target {current_target_dev_name}")
                    else:
                        # Force an update if the source or target wasn't matched
                        for dev_id in sionna_structure["sionna_location_db"]:
                            if dev_id[:3] == "car":
                                car_name = f"car_{dev_id[3:]}"
                                if sionna_structure["scene"].get(car_name):
                                    from_sionna = sionna_structure["scene"].get(car_name)
                                    new_position = [sionna_structure["SUMO_live_location_db"][dev_id]["x"],
                                                    sionna_structure["SUMO_live_location_db"][dev_id]["y"],
                                                    sionna_structure["SUMO_live_location_db"][dev_id]["z"]]
                                    from_sionna.position = new_position
                                    # Update Sionna location database with new positions
                                    sionna_structure["sionna_location_db"][dev_id] = {"x": new_position[0],
                                                                                      "y": new_position[1],
                                                                                      "z": new_position[2], 
                                                                                      "angle": sionna_structure["SUMO_live_location_db"][dev_id]["angle"]}
                                    # Update antenna positions
                                    if sionna_structure["scene"].get(dev_id+"_tx_antenna"):
                                        sionna_structure["scene"].get(dev_id+"_tx_antenna").position = \
                                    ...
                                    if sionna_structure["scene"].get(dev_id+"_rx_antenna"):
                                        sionna_structure["scene"].get(dev_id+"_rx_antenna").position = \
                                            [new_position[0] + sionna_structure["antenna_displacement"][0],
                                             new_position[1] + sionna_structure["antenna_displacement"][1],
                                             new_position[2] + sionna_structure["antenna_displacement"][2]]
                                        if sionna_structure["verbose"]:
                                            print(f"Forced update for {dev_id} and its RX antenna in the scene.")
                                else:
                                    print(f"ERROR: no {dev_id} in the scene for forced update, use Blender to check")

                        # Re-do matching with updated locations
                        t = time.time()
                        matched_paths = match_rays_to_devs(paths, sionna_structure)
                        if sionna_structure["time_checker"]:
                            print(f"Matching rays to devs (double exec) took: {(time.time() - t) * 1000} ms")
                        if current_source_dev_name not in sionna_structure["rays_cache"]:
                            sionna_structure["rays_cache"][current_source_dev_name] = {}
                        if current_target_dev_name in matched_paths[current_source_dev_name]:
                            sionna_structure["rays_cache"][current_source_dev_name][current_target_dev_name] = \
                                matched_paths[current_source_dev_name][current_target_dev_name]

    return None

def get_path_loss(dev1_id, dev2_id, sionna_structure):
    t = time.time()

    #print(sionna_structure["rays_cache"])

    # Was the requested value already calculated?
    if dev1_id not in sionna_structure["rays_cache"] or dev2_id not in sionna_structure["rays_cache"][dev1_id]:
        if sionna_structure["verbose"]:
            print(f"Pathloss calculation requested for {dev1_id}-{dev2_id}: rays not computed yet.")
        
        compute_rays(sionna_structure)
    
    if sionna_structure["verbose"]:
        print(f"Pathloss calculation requested for {dev1_id}-{dev2_id}: rays retreived from cache.")
    
    path_coefficients = sionna_structure["rays_cache"][dev1_id][dev2_id]["path_coefficients"]
    ...
    if total_cir > 0:
        ...
        if sionna_structure["verbose"]:
            print(
                f"Pathloss calculation failed for {dev1_id}-{dev2_id}: got infinite value (not enough rays). Returning 300 dB.")
...
def manage_path_loss_request(message, sionna_structure):
    try:
        ...
        # Getting each dev_id, the origin is marked as 0 - ns-3 marks a dev at [0,0,0] when still outside the simulation
        if dev_a_id[3:] == "0":
            dev_a_id = "origin" 
        ...
        if dev_a_id == "origin" or dev_b_id == "origin":
            # If any, ignoring path_loss requests from the origin, used for statistical calibration
            path_loss_value = 0
        else:
            t = time.time()
            path_loss_value = get_path_loss(dev_a_id, dev_b_id, sionna_structure)

        return path_loss_value

    except (ValueError, IndexError) as e: # , KeyError
        print(f"EXCEPTION - Error processing path_loss request: {e}")
        return None

def get_delay(dev1_id, dev2_id, sionna_structure):
    t = time.time()
    # Check and compute rays only if necessary
    if dev1_id not in sionna_structure["rays_cache"] or dev2_id not in sionna_structure["rays_cache"][dev1_id]:
        compute_rays(sionna_structure)

    delays = np.abs(sionna_structure["rays_cache"][dev1_id][dev2_id]["delays"])
...
def manage_delay_request(message, sionna_structure):
    try:
        ...
        if dev_a_id == "origin" or dev_b_id == "origin":
            ...
            delay = get_delay(dev_a_id, dev_b_id, sionna_structure)
...
def manage_los_request(message, sionna_structure):
    ...
    try:
        ...
        if dev_a_id == "origin" or dev_b_id == "origin":
            ...
            los = sionna_structure["rays_cache"][dev_a_id][dev_b_id]["is_los"]
...
def main():
    ...
    # Scenario
    parser.add_argument('--path-to-xml-scenario', type=str, default='scenarios/SionnaExampleScenario/munich.xml',
    ...
    parser.add_argument('--angle-threshold', type=float, help='Angle threshold for ray tracing', default=0.1)
    ...
    log_filename = "src/sionna/sionna_log.csv"
    ...
    while True:
        ...
        if message.startswith("LOC_UPDATE:"):
            ...
            updated_dev = manage_location_message(message, sionna_structure)
            if updated_dev is not None:
                response = "LOC_CONFIRM:" + str(updated_dev)
        ...
        if message.startswith("SHUTDOWN_SIONNA"):
            if pl1:
                ...
                my_cam = Camera(position=[-0, -0, 1900], orientation=np.array([0, np.pi/2, -np.pi/2])) # Orientation: N/S, W/E, Counter/Clockwise
                sionna_structure["scene"].render(camera=my_cam, resolution=[3600, 3000], num_samples=100); # Increase num_samples to increase image quality
                plt.show()
                plt.savefig('scene_map_ns3rt_new.png')

                my_cam = Camera(position=[-0, -0, 500], orientation=np.array([0, np.pi/2, -np.pi/2])) # Orientation: N/S, W/E, Counter/Clockwise
                sionna_structure["scene"].render(camera=my_cam, resolution=[3600, 3000], num_samples=100); # Increase num_samples to increase image quality
                plt.show()
                plt.savefig('scene_map_ns3rt_new-zoomin.png')
            elif pl2:
                print("Rendering scene")
                #my_cam = Camera(position=[-250,250,150], look_at=[-15,30,28])
                my_cam = Camera(position=[100,550,100], look_at=[65,200,10])
                sionna_structure["scene"].render(camera=my_cam, resolution=[650, 500], num_samples=1000); # Increase num_samples to increase image quality
                plt.show()
                plt.savefig('scene_map_ns3rt_new-view_hd.png')
                
                for i in range(9,38):
                    c_n = sionna_structure["scene"].get("car_"+str(i))
                    print("Car " +str(i)+ " position: ", c_n.position)
                    print("Car " +str(i)+ " orientation (alpha, beta, gamma) [rad]: ", c_n.orientation)

            end_time = time.time()
            elapsed_time = (end_time - start_time)/60
            print(f"Elapsed time: {int(round(elapsed_time))} minutes")
